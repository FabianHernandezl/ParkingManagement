...
public class ParkingLotViewInternal extends JInternalFrame {
    ...
    // Tipos de vehículo predefinidos
    private VehicleType standardType;
    private VehicleType motorcycleType;
    private VehicleType disabledType;
    ...
    public ParkingLotViewInternal() {
        ...
        JLabel lblCarFee = new JLabel(" Automóviles: $" + String.format("%.2f", standardType.getFee()));
        lblCarFee.setForeground(new Color(0, 100, 0));
        feePanel.add(lblCarFee);
        
        JLabel lblBikeFee = new JLabel(" Motocicletas: $" + String.format("%.2f", motorcycleType.getFee()));
        lblBikeFee.setForeground(new Color(0, 100, 0));
        feePanel.add(lblBikeFee);
        
        JLabel lblDisabledFee = new JLabel(" Discapacitados: $" + String.format("%.2f", disabledType.getFee()));
    ...
    private void initializeVehicleTypes() {
        standardType = new VehicleType(1, "Automóvil", 4, 5.0f);
        motorcycleType = new VehicleType(2, "Motocicleta", 2, 2.5f);
        disabledType = new VehicleType(3, "Discapacitado", 4, 0.0f);
    }
        ...
        try {
            ...
            if (success != null) {
                JOptionPane.showMessageDialog(this, 
                    ...
                    "• Adaptados: " + config.getDisabledSpaces() + "\n" +
                    "• Motocicletas: " + config.getMotorcycleSpaces() + "\n" +
                    "• Estándar: " + config.getStandardSpaces(),
                    "Registro Exitoso", 
    ...
    private SpaceConfiguration askForSpaceConfiguration(int totalSpaces) {
        ...
        JLabel lblStandard = new JLabel("Espacios estándar: ");
        ...
        panel.add(new JLabel("Espacios para discapacidad:"));
    ...
    private ParkingLot createParkingLotFromForm(SpaceConfiguration config) {
        ...
        for (int i = 0; i < config.getDisabledSpaces(); i++) {
            spaces[spaceIndex] = new Space(spaceIndex + 1, true, false, disabledType);
            spaceIndex++;
        }
        ...
        for (int i = 0; i < config.getStandardSpaces(); i++) {
            spaces[spaceIndex] = new Space(spaceIndex + 1, false, false, standardType);
            spaceIndex++;
        }
    ...
    private ParkingLot updateExistingParkingLot(ParkingLot existing, SpaceConfiguration config) {
        ...
        if (newNumberOfSpaces > oldSpaces.length) {
            ...
            for (int i = 0; i < newDisabled && spaceIndex < newNumberOfSpaces; i++) {
                newSpaces[spaceIndex] = new Space(spaceIndex + 1, true, false, disabledType);
                spaceIndex++;
            }
            ...
            while (spaceIndex < newNumberOfSpaces) {
                newSpaces[spaceIndex] = new Space(spaceIndex + 1, false, false, standardType);
                spaceIndex++;
            }
        }
    ...
    private void showParkingLotDetailsDialog(ParkingLot parkingLot) {
        ...
        if (parkingLot.getSpaces() != null) {
            ...
            details.append("─────────────────────────\n");
            details.append("• Adaptados discapacidad: ").append(disabledSpaces).append("\n");
            details.append("• Para motocicletas: ").append(motorcycleSpaces).append("\n");
            details.append("• Estándar: ").append(parkingLot.getSpaces().length - disabledSpaces - motorcycleSpaces).append("\n");
    ...
    private void showConfigurationDialog() {
        JOptionPane.showMessageDialog(this,
            ...
            "Tarifas por hora:\n" +
            "• Automóviles: $" + String.format("%.2f", standardType.getFee()) + "\n" +
            "• Motocicletas: $" + String.format("%.2f", motorcycleType.getFee()) + "\n" +
            "• Discapacitados: $" + String.format("%.2f", disabledType.getFee()) + "\n\n" +
            "Distribución predeterminada:\n" +
            "• 10% espacios para discapacidad\n" +
    ...
    class SpaceConfiguration {
        ...
        public SpaceConfiguration(int disabledSpaces, int motorcycleSpaces, int standardSpaces) {
            this.disabledSpaces = disabledSpaces;
            this.motorcycleSpaces = motorcycleSpaces;
            this.standardSpaces = standardSpaces;
        }
        
        public int getDisabledSpaces() { return disabledSpaces; }
        public int getMotorcycleSpaces() { return motorcycleSpaces; }
        public int getStandardSpaces() { return standardSpaces; }
        public int getTotalSpaces() { return disabledSpaces + motorcycleSpaces + standardSpaces; }
    }
